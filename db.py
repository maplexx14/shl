...
class BotDB:
    ...
    def find_anketi(self, user_id, interest, city, age):
        ...
        result = self.cursor.execute("SELECT * FROM `anketi` WHERE `users_id` != ? AND `gender` = ? AND `city` = ? AND"
                                     "`age` BETWEEN ? AND ?", (self.get_user_id(user_id), gender, city.title(), int(age)
                                                                                                    - 1, int(age) + 1))
